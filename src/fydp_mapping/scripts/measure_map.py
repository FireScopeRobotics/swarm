...
def callback_isaac(msg):
    ...
    # In square meters
    mapped_footage = (occupany_map == 0).sum() * (map_res**2)

    #if (rospy.get_time() - start_time) < 180.0:
    print(f"Isaac Map is: {mapped_footage} m^2")
    print(f"Isaac Map is: {mapped_footage * 10.76391042} ft^2")
    print("-----")
    # else:
    #     print("3 mins complete: Saving map and shutting down")
    #     rospy.signal_shutdown("3 mins complete: Saving map and shutting down")
...
def listener():
    ...
    rospy.Subscriber("/map_isaac", OccupancyGrid, callback_isaac)
    rospy.spin()
