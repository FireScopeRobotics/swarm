...
int main(int argc, char * argv[])
{
  ...
  const auto new_tb3_0_map_pub = nh.advertise<nav_msgs::OccupancyGrid>("new_carter1_map", 100);
  const auto map_meta0_sub = nh.subscribe<nav_msgs::OccupancyGrid>("carter1/map", 100, map0Callback);

  const auto new_tb3_1_map_pub = nh.advertise<nav_msgs::OccupancyGrid>("new_carter2_map", 100);
  const auto map_meta1_sub = nh.subscribe<nav_msgs::OccupancyGrid>("carter2/map", 100, map1Callback);
  
  const auto new_tb3_2_map_pub = nh.advertise<nav_msgs::OccupancyGrid>("new_carter3_map", 100);
  const auto map_meta2_sub = nh.subscribe<nav_msgs::OccupancyGrid>("carter3/map", 100, map2Callback);

  ros::Rate loop_rate(100);

  while (ros::ok())
  {
    if ( (slam0_map.data.size() != 0) || (slam1_map.data.size() != 0) || (slam2_map.data.size() != 0) || slam0_map.info.origin.position.x !=0 || slam1_map.info.origin.position.x !=0 || slam2_map.info.origin.position.x !=0)
    {
      // Create the new larger maps for each robot
      nav_msgs::OccupancyGrid new_tb3_0_map, new_tb3_1_map, new_tb3_2_map;
      new_tb3_0_map.header.frame_id = "carter1_map";
      ...
      new_tb3_1_map.header.frame_id = "carter2_map";
      ...
      new_tb3_2_map.header.frame_id = "carter3_map";
      new_tb3_2_map.info.resolution = 0.05;
    ...
    /////////////////////////////////////////////////////////////////////////////
      ...
      for (int item_counter=0; item_counter < slam2_map.info.height; item_counter++)
      {
        ...
        {
          new_tb3_2_map.data.push_back(slam2_map.data[c2]);
          c2++;
        }
